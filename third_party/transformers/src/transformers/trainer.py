...
class MyTrainerState:
    ...
    def save_to_json(self, json_path: str):
        ...
        s = dataclasses.asdict(self)
        del s['samples_loss']
        json_string = json.dumps(s, indent=2, sort_keys=True) + "\n"
...
class MyTrainerControl:
    ...
    Args:
        ...
        should_log (`bool`, *optional*, defaults to `False`):
            ...
            If `True`, this variable will be set back to `False` at the beginning of the next step.
        should_use_smaller_samples (`bool`, *optional*, defaults to `False`):
    """
    ...
    should_use_fewer_samples: bool = False

    def _new_training(self):
        """Internal method that resets the variable for a new training."""
        self.should_training_stop = False
        self.use_smaller_samples = False
...
class Trainer:
    ...
    def __init__(
        ...
        self.state = MyTrainerState(
            is_local_process_zero=self.is_local_process_zero(),
            is_world_process_zero=self.is_world_process_zero(),
        )

        self.control = MyTrainerControl()
    ...
    def _set_signature_columns_if_needed(self):
        if self._signature_columns is None:
            ...
            self._signature_columns += list(["idx"])
    ...
    def get_train_dataloader(self, subset=None) -> DataLoader:
        ...
        if getattr(self, 'original_train_dataset', None) is None:
            train_dataset = self.train_dataset
        else:
            train_dataset = self.original_train_dataset
        ...
        if subset is not None:
            ...
            train_dataset_small = torch.utils.data.Subset(train_dataset, subset)
            if getattr(self, 'original_train_dataset', None) is None:
                self.original_train_dataset = copy.deepcopy(self.train_dataset)
            self.train_dataset = train_dataset_small
    ...
    def _inner_training_loop(
        ...
        if resume_from_checkpoint is not None and os.path.isfile(
            ...
            self.state = MyTrainerState.load_from_json(os.path.join(resume_from_checkpoint, TRAINER_STATE_NAME))
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            if self.control.should_use_fewer_samples:
                ...
                train_dataloader = self.get_train_dataloader(mask)
                self.train_dataloader = train_dataloader
                model.zero_grad()
                self.control.should_use_fewer_samples = False
